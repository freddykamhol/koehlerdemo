...
if(sessionStorage.getItem('koehler-demo-access')!=='granted'){
  ...
  gate.innerHTML=`<div class="access-card"><span class="access-label">KA Technologies · Kunden-Demo</span><img src="/assets/Logo.png" alt="Köhler GmbH"><h1>Geschützter<br><em>Projektzugang.</em></h1><p>Bitte geben Sie das Passwort ein, um die Website-Demo anzusehen.</p><form><label for="demo-password">Passwort</label><div class="password-input"><input id="demo-password" type="password" autocomplete="current-password" autofocus placeholder="Passwort eingeben"><button type="button" aria-label="Passwort anzeigen">Anzeigen</button></div><small class="access-error" aria-live="polite"></small><button class="access-submit" type="submit">Demo öffnen <span>→</span></button></form><small class="access-credit">Konzept & Umsetzung · KA Technologies</small></div>`;
